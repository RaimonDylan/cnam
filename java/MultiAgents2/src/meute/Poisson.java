...
public class Poisson extends CObject {
    ...
    public static final double DISTANCE_COMBAT_MIN = 3;
    public static final double DISTANCE_COMBAT_MIN_CAREE = 9;
    public static final double DISTANCE_SELECTION_MIN = 10;
    public static final double DISTANCE_SELECTION_MIN_CARRE = 100;
    
    
    public static final double VOISINAGE = 100;
    public static final double VOISINAGE_CARRE = 10000;

    public static final double LONGUEUR_INITIALE = 30;
    ...
    protected int famille;
    protected int energie;
    protected boolean estSelectionne;
    
    // Méthodes
    public Poisson(double _x, double _y, double _dir, double _longueur, int _famille) {
        posX = _x;
        posY = _y;
        longueur = _longueur;
    ...
    protected boolean DansAlignement(Poisson p) {
        ...
        return ((p.famille == famille) && distanceCarre < DISTANCE_MAX_CARRE
        		&& distanceCarre > DISTANCE_MIN_CARRE);
    }
    
    protected double DistanceAuMur(
    		double murXMin, double murYMin, double murXMax, double murYMax) {
    ...
    protected boolean EviterPoissons(Poisson[] poissons) {
        ...
        for (Poisson poisson : poissons) {
            if (DistanceCarre(poisson) < distanceCarre && !poisson.equals(this)) {
                ...
                // Combat
                if (distanceCarre < DISTANCE_COMBAT_MIN_CAREE) {
                	if ((p.energie > 0) && (energie > 0)) {
                		if (Ocean.generateur.nextDouble() >= 0.5) {
	                		p.energie -= 2;
	                	} else {
	                		energie -= 2;
	                	}
    ...
    protected void MiseAJour(
    		Poisson[] poissons,
    		ArrayList<ZoneAEviter> obstacles,
    		double largeur,
    		double hauteur) {
    ...
    public void selectionneSiPossible(int x, int y)  {
    	if (DistanceCarre(x, y) <= DISTANCE_SELECTION_MIN_CARRE) {
